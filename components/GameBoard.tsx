                ...
                return (
                    <div className="flex flex-col items-center gap-8 animate-in slide-in-from-bottom">
                        <div className="text-center">
                            <h2 className="text-3xl font-bungee text-emerald-500">Bỏ phiếu kín</h2>
                            <p className="text-slate-400 text-sm mt-1">
                                Đã bỏ phiếu: <span className="text-white font-bold">{totalVotes}/{totalPlayers}</span>
                            </p>
                        </div>
                        ...
                        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 w-full">
                            {(Object.values(roomData.players) as Player[]).map(p => (
                                <button 
                                    ...
                                    className={`p-6 rounded-3xl border transition-all flex flex-col items-center gap-3 relative
                                    ...
                                    {(Object.values(roomData.players) as Player[])
                                        ...
                                        .map(p => (
                                            <div key={p.id} className="flex justify-between items-center text-sm">
                                                <span>{p.name}</span>
                                                <span className="font-bold text-white bg-slate-700 px-2 rounded">{p.voteCount || 0} phiếu</span>
                                            </div>
                                        ))
                                    }
                                </div>
                        ...
                        showOpponentSelector ? (
                            ...
                            <div className="flex flex-col gap-4 w-full max-w-md animate-in slide-in-from-right">
                                ...
                                <button onClick={() => handleSelectMinigame(MinigameType.MEMORY)} className="p-4 bg-slate-800 hover:bg-emerald-600 border border-emerald-500/50 rounded-2xl flex items-center gap-4 transition-all">
                                    <span className="text-3xl"><Bomb className="text-rose-500 inline" size={32}/></span><div className="text-left"><div className="font-bold text-white">Lật Thẻ Tử Thần</div><div className="text-xs text-slate-400">Né bom hoặc uống!</div></div>
                                </button>
                                <button onClick={() => handleSelectMinigame(MinigameType.TAP_WAR)} className="p-4 bg-slate-800 hover:bg-orange-600 border border-orange-500/50 rounded-2xl flex items-center gap-4 transition-all">
                                    <span className="text-3xl">🥊</span><div className="text-left"><div className="font-bold text-white">Loạn Đả Màn Hình</div><div className="text-xs text-slate-400">Bấm gãy tay thì thôi!</div></div>
                                </button>
